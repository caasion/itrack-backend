...
const mergeList = (existing, incoming, limit = 10) => {
...
export const getProfile = async (userId) => {
    ...
    try {
        const response = await fetch(`${settings.BACKBOARD_BASE_URL}/memory/itrack:profile:${userId}`, {
            method: "GET",
            headers: authHeaders(),
...
export const updateProfile = async (userId, signals) => {
...
export const getDwellCount = (userId) => {
    return dwellCounts.get(userId) ?? 0;
};
export const clearProfile = (userId) => {
