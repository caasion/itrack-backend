...
const generateWithModelFallback = async (parts) => {
    ...
    for (const modelName of orderedCandidates) {
        try {
            ...
            if (finishReason !== "STOP" && finishReason !== "MAX_TOKENS") {
                ...
                throw new Error(`[Gemini] Response blocked or empty — finishReason=${finishReason} safetyRatings=${safetyRatings} promptFeedback=${promptFeedback}`);
            }
...
export const identifyAndUpdate = async (screenshotB64, userId, pageUrl, pageTitle) => {
    let signals = FALLBACK_SIGNALS;
    try {
        const imagePart = {
            inlineData: {
                mimeType: "image/jpeg",
        ...
        const prompt = IDENTIFY_PROMPT.replace("{pageUrl}", pageUrl).replace("{pageTitle}", pageTitle ?? "unknown");
        ...
        const cleanSignal = (s) => s.replace(/\s*\([^)]*\)/g, "").trim();
        signals = {
            ...raw,
        ...
        if (stack)
            console.warn("[Gemini] Stack:", stack.split("\n").slice(0, 4).join(" | "));
    }
    try {
